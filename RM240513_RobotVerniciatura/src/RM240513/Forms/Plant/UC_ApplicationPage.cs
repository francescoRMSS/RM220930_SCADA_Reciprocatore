...
namespace RM.src.RM240513
{
    ...
    public partial class UC_ApplicationPage : UserControl
    {
                ...
                try
                {
                    ...
                    if (!isContained)
                    {
                        ...
                        if (result)
                        {
                            ...
                            if (CustomMessageBox.Show(MessageBoxTypeEnum.WARNING, "Modificare ID applicazione selezionata in " + id + " ?") 
                                == DialogResult.OK)
                            {
        ...
        private void ClickEvent_loadProgram(object sender, EventArgs e)
        {
            ...
            UC_FullDragModePage.debugCurrentIndex = -1; // Reset dell'indice posizione usato in monitoring
            RobotManager.currentIndex = -1; // Reset dell'indice corrente della posizione che riproduce il Robot
            RobotManager.pauseCycleRequested = false; // Reset della richiesta di pausa ciclo
            RobotManager.stopCycleRoutine = true; // Richiesta stop cicLo Robot
            UC_HomePage.index = 0; // Reset indice della posizione da selezionare su lw_positions
