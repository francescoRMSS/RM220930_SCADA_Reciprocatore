...
namespace RM.src.RM240513
{
    ...
    public class TaskManager
    {
        ...
        private async Task CheckTasks(CancellationToken token)
        {
            ...
            {
                log.Error("[TASK] CheckTasks - ERRORE");
            }
            finally
            {
                _checkTasksCTS = null;
        ...
        /// <returns></returns>
        private static async Task TaskStructure(CancellationToken token)
        {
            ...
            {
                // Cosa fare quando il task viene fermato tramite task manager
                throw;
            }
            catch (Exception)
            {
                // Cosa fare quando il task va in eccezione generica
                throw;
            }
            finally
            {
                // Rilascio risorse usate
            }
        }
    }
}
